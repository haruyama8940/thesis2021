...
\begin{document}
  ...
  \title{　　　　　　視覚と行動のEnd-to-End学習により経路追従行動を　　　　　　オンラインで模倣する手法の提案}
    ...
    カメラ画像と目標方向をEnd-to-End学習によって模倣学習を行う．
    学習フェーズでは3つのカメラから画像を取得する．
    % 学習器の訓練に用いるデータは(入力:カメラ画像,目標方向　目標出力:角速度)
    地図ベースの制御器は，
    ...
    \begin{table}[H]
        ...
        \begin{center}
            ...
            \begin{tabular}{|c|c|}
                \hline
                Target direction & Date\\ \hline
                continue & $[100,0,0,0]$ \\ \hline
                go straight & $5/5$ \\ \hline
                A - C (turn left) & $4/5$ \\ \hline
                A - D (turn right) & $5/5$ \\ \hline
            \end{tabular}
        \end{center}
    \end{table}
    ...
    % Start-A(continue)，A-B(go straight)，A-C(turn left)
    % ，A-D(turn right)を入力する．
    実験条件はテストフェーズにおいて「壁に衝突せず,指定したゴールへ到達」を成功，
    「目標方向とは異なった経路を選択,または壁に衝突」を失敗とする．
    \begin{center}
        \begin{figure}[h]
            \centering
            \includegraphics[width = 7cm]{./fig/zyuziroute.pdf}
   ...
   \begin{table}[H]
    ...
    \begin{center}
        ...
        \begin{tabular}{|c|c|}
            ...
            Start - A (continue) & $5/5$ \\ \hline
            A - B (go straight) & $5/5$ \\ \hline
            A - C (turn left) & $4/5$ \\ \hline
            A - D (turn right) & $5/5$ \\ \hline
        \end{tabular}
    \end{center}
