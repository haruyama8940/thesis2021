...
学習フェーズ
テストフェーズ
本研究で対象とするロボットの構造，搭載するセンサをTable\ref{tb:hikaku}に定義する
...
%     \cline{1-3}
%                                      & conventional method     & Proposed method          &  &  \\ \cline{1-3}
%     Input                               & Camera Image    & Camera Image, Command   &  &  \\ \cline{1-3}
%     Output                               & Angular velocity      & Angular velocity           &  &  \\ \cline{1-3}
%     Route                             & Circumnavigate a certain path & Route according to command &  &  \\ \cline{1-3}
%     \multicolumn{1}{|l|}{Change of route} & ×        & ○             &  &  \\ \cline{1-3}
%      \end{tabular}
...
\section{地図ベースの制御器}
    ...
    \begin{figure}[h]
        \centering
        \includegraphics[width = 15cm]{./figs/navigation.pdf}
